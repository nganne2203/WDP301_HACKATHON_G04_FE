...
import { useRoundsQuery } from '@/hooks/queries/useCommonQueries';
...
export function CompetitionDetailsDialog({
  ...
  const roundsQuery = useRoundsQuery(
    { competitionId: competition?.id, page: 1, limit: 100 },
    { enabled: open && Boolean(competition?.id) }
  );
  const rounds = roundsQuery.data || [];
  const preliminaryRounds = rounds.filter((round) => round.roundType === 'PRELIMINARY').length;
  const finalRounds = rounds.filter((round) => round.roundType === 'FINAL').length;
  const roundsSummary = roundsQuery.isLoading
    ? 'Loading rounds...'
    : rounds.length > 0
      ? `${rounds.length} round(s) (${preliminaryRounds} preliminary, ${finalRounds} final)`
      : '2 rounds (1 preliminary, 1 final) — default structure';
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium mt-1">{roundsSummary}</p>
              </div>
            </div>
